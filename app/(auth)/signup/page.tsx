...
function SignUpForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!organizationName && !isNapaEmail) { setError('Please select your organization'); return }
    ...
    try {
      ...
      if (data.approvalStatus === 'pending') {
        ...
        setTimeout(async () => {
          const result = await signIn.email({ email, password, callbackURL: '/pending-approval' })
          if (!result.error) { router.push('/pending-approval'); router.refresh() }
  ...
  return (
    <div className="grid min-h-svh lg:grid-cols-2">
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-6">
              <FieldGroup>
                ...
                <Field>
                  ...
                  <Input id="email" type="email" placeholder="chair@napahq.org" value={email} onChange={(e) => setEmail(e.target.value)} disabled={isLoading || !!emailParam} required />
                </Field>

                <Field>
                  <FieldLabel htmlFor="password">Password</FieldLabel>
                  <div className="relative">
                    <Input id="password" type={showPassword ? "text" : "password"} placeholder="Create a password (min 8 characters)" value={password} onChange={(e) => setPassword(e.target.value)} disabled={isLoading} required className="pr-10" />
                ...
                <Field>
                  ...
                  <div className="relative">
                    <Input id="confirmPassword" type={showConfirmPassword ? "text" : "password"} placeholder="Confirm your password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} disabled={isLoading} required className="pr-10" />
                ...
                {isNapaEmail ? (
                  <Field>
                    ...
                    <Input id="organization" value="National APIDA Panhellenic Association" disabled className="bg-muted" />
                    <FieldDescription>Automatically assigned based on your email domain</FieldDescription>
                  </Field>
                ) : isInvited && orgParam ? (
                  <Field>
                    <FieldLabel htmlFor="organization">Organization</FieldLabel>
                    <Input id="organization" value={orgParam} disabled className="bg-muted" />
                    <FieldDescription>Pre-assigned by your organization admin</FieldDescription>
                  </Field>
                ) : (
                  <Field>
                    <FieldLabel htmlFor="organization">Organization</FieldLabel>
                    <Select value={organizationName} onValueChange={setOrganizationName} disabled={isLoading || loadingOrgs} required>
                      ...
                      <SelectContent sideOffset={5}>
                        {organizations.length === 0 ? (
                          ...
                          organizations.map((org) => (
                            <SelectItem key={org.id} value={org.organization_name}>{org.organization_name}</SelectItem>
                          ))
                        )}
                      </SelectContent>
                    </Select>
                ...
                <Field orientation="horizontal">
                  ...
                  <label htmlFor="terms" className="text-sm leading-tight cursor-pointer">
                    I agree to the{' '}
                    <button type="button" onClick={() => setTermsDialogOpen(true)} className="text-primary hover:text-primary/80 underline underline-offset-4">Terms</button>
                    {' '}and{' '}
                    <button type="button" onClick={() => setPrivacyDialogOpen(true)} className="text-primary hover:text-primary/80 underline underline-offset-4">Privacy Policy</button>
                  </label>
                </Field>

                <Field>
                  <Button type="submit" className="w-full" disabled={isLoading || loadingOrgs}>
                ...
                <FieldDescription className="text-center">
                  ...
                  <a href="/login" className="text-primary hover:text-primary/80 underline underline-offset-4">Sign in</a>
                </FieldDescription>
              </FieldGroup>
            </form>
          </div>
      ...
      <div className="bg-primary relative hidden lg:flex lg:flex-col lg:items-center lg:justify-center lg:p-12">
        <div className="text-center space-y-6">
          <div className="flex justify-center"><NapaAuthLogo size="xl" /></div>
          <h2 className="text-3xl font-bold text-black">NAPA Resource Hub</h2>
          <p className="text-lg text-black/80 max-w-md mx-auto">
            A shared resource library for NAPA organizations to collaborate and share best practices.
          </p>
        </div>
        ...
        <DialogContent className="sm:max-w-2xl max-h-[80vh] overflow-y-auto">
          ...
          <div className="prose prose-sm max-w-none space-y-5 text-gray-700">
            ...
            <section>
              ...
              <p>This platform is for authorized representatives of NAPA member organizations. To use it, you must be at least 18 years old, register with a valid email address, provide your real name and organization affiliation, and be approved by your organization&apos;s administrator or a NAPA administrator.</p>
            </section>
            <section>
              <h3 className="text-base font-semibold text-foreground">3. Use It Responsibly</h3>
              <p>You may browse, search, download, and upload shared resources. You may not use the platform for anything illegal, upload harmful content, share logins, or misrepresent yourself.</p>
            </section>
          </div>
        </DialogContent>
      </Dialog>
        ...
        <DialogContent className="sm:max-w-2xl max-h-[80vh] overflow-y-auto">
          ...
          <div className="prose prose-sm max-w-none space-y-5 text-gray-700">
            <section>
              ...
              <p>This Privacy Policy explains what information we collect when you use the NAPA Resource Hub, how we use it, and your rights.</p>
            </section>
            <section>
              <h3 className="text-base font-semibold text-foreground">2. What We Collect</h3>
              <p>We collect your email, name, organization affiliation, and activity logs. We do not use analytics trackers, collect payment info, or sell your data.</p>
            </section>
          </div>
        </DialogContent>
      </Dialog>
