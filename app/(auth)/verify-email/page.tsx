...
export default function VerifyEmailPage() {
  ...
  const handleSendCode = async () => {
    ...
    setIsSending(true)
    try {
      const result = await emailOtp.sendVerificationOtp({ email: session.user.email, type: 'email-verification' })
      if (result.error) {
  ...
  const handleVerify = async (codeToVerify?: string) => {
    ...
    if (!session?.user?.email) { toast.error('Session expired. Please log in again.'); router.push('/login'); return }

    setIsVerifying(true)
    try {
      const result = await emailOtp.verifyEmail({ email: session.user.email, otp: verificationCode })
      if (result.error) {
      ...
      await fetch('/api/v2/auth/otp/verify-and-update', { method: 'POST' })
          ...
          <CardContent>
            <div className="space-y-6">
              ...
              <div className="text-center">
                ...
                <Button variant="link" onClick={handleSendCode} disabled={isSending || resendCooldown > 0} className="text-primary">
